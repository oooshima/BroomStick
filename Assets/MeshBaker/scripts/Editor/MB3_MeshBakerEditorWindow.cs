	...
	void drawTabAnalyseScene(){
		...
		if (sceneAnalysisResults.Count > 0){
			...
			for(int i = 0; i < sceneAnalysisResults.Count; i++){
				...
				string descr = String.Format("Objs={0} AtlasIndex={1} {2}", gows.Count, gows[0].atlasIndex, gows[0].GetDescription(GameObjectFilterInfo.filters,gows[0]));
...
//		Profile.StartProfile("listMaterialsInScene1");
        ...
        Dictionary<GameObjectFilterInfo, List<List<GameObjectFilterInfo>>> gs2bakeGroupMap = sortIntoBakeGroups3(gameObjects, objsNotAddedToBaker);
...
//		}		
    ...
    Dictionary<GameObjectFilterInfo, List<List<GameObjectFilterInfo>>> sortIntoBakeGroups3(List<GameObjectFilterInfo> gameObjects, List<GameObjectFilterInfo> objsNotAddedToBaker){
